...
using BillingSystemV4.Infrastructure;
...
namespace BillingSystemV4.Controllers
{
    public class NotificationsController : Controller
    {
        ...
        public async Task<IActionResult> Index(string id)
        {
            ...
            var Accesstoken = await AuthenticateOutboundRequestAsync("a5ec227e-1161-487d-8e4b-168353d1a844", KeyVaultOptions.Value.ClientId, KeyVaultOptions.Value.ClientSecret);
            var GetToken = JsonConvert.DeserializeObject<Token>(Accesstoken);
            var GetInformation = await GetCallInformation(GetToken.access_token, id);
            ...
            catch (Exception ex)
            {
                ...
                logger.LogError($"ParsingNotification: { ex.Message }" + "------ Call ID:" + id);
        ...
        public async Task<IActionResult> Listen([FromQuery] string validationToken = null)
        {
            if (string.IsNullOrEmpty(validationToken))
            {
                try
                {
                    ...
                    var collection = await System.Text.Json.JsonSerializer.DeserializeAsync<NotificationModel>(Request.Body, options);
